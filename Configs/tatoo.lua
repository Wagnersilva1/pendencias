...
function interpCamera(cameraName)
	if cameras[cameraName] and cameraName ~= activeCam then
		...
		local tempCam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", coord.x, coord.y, cam.coords.z + pointCoords.z, 0, 0,
			0, 50.0)
...
local tattooShops = {
    ...
    [7] = { coords = vec3(-1052.76,-1180.61,2.27), h = 110.56 },
    [8] = { coords = vec3(-1057.85,-1183.97,2.27), h = 164.41 },
    [9] = { coords = vec3(-1064.24,-1187.18,2.27), h = 204.01 },
	[new] = { coords = vec3(854.67,474.4,139.65), h = 39.69 },
}
...
mainThread = function()
	...
	-- Adicionar interações para cada loja de tatuagem
	for tattooShopId, v in pairs(tattooShops) do
		if v and v.coords then
			exports.interact:AddInteraction({
				coords = v.coords,
				distance = 6.0,
				interactDst = 1.5,
				id = "tattooshop_" .. tattooShopId,
				name = "tattooshop_" .. tattooShopId,
				groups = v.perm and { [v.perm] = true } or nil,
				options = {
					{
						label = "Acessar Loja de Tatuagens",
						action = function(entity, coords, args)
							if v.perm then
								if func.checkPermission(v.perm) then
									startTattoos(tattooShopId)
								end
							else
								startTattoos(tattooShopId)
							end
...
function initTattooShop()
	...
	if tattooId and tattooShops[tattooId] then
		SetEntityCoordsNoOffset(ped, tattooShops[tattooId].coords)
		SetEntityHeading(ped, tattooShops[tattooId].h)
	end
	freezeAnim("move_f@multiplayer", "idle")
...
RegisterCommand("loja4", function() -- abrir menu do tattooshop (admin)
...
AddEventHandler("reloadtattos", reloadTattoos)
RegisterNetEvent("forcereloadtattos")
AddEventHandler("forcereloadtattos", forceReloadTattoos)
